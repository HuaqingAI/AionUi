...
!macro AIONUI_READ_LAST_INNER_FAILURE
  ...
  StrCpy $AionUiInnerFailureSummary "No specific locking process was identified. Close 华青智能助手, terminals, editors, and file managers opened in the install folder."
  nsExec::ExecToStack `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
    ...
    $$summary = 'No specific locking process was identified. Close 华青智能助手, terminals, editors, and file managers opened in the install folder.'; \
    $$code = ''; \
...
!macro AIONUI_REPAIR_INSTALLED_UNINSTALLER
  ...
  InitPluginsDir
  StrCpy $AionUiBundledUninstaller "$PLUGINSDIR\${APP_FILENAME}-fixed-uninstaller.exe"
  SetOverwrite on
  File "/oname=$PLUGINSDIR\${APP_FILENAME}-fixed-uninstaller.exe" "${UNINSTALLER_OUT_FILE}"
