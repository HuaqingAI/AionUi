      ...
      try { \
        ...
        if ($$resources.Count -eq 0) { \
          if ($$installerSelfLock -and $$installerPid -gt 0) { \
            $$lockerText = '华青智能助手安装程序(' + $$installerPid + ')'; \
            [System.IO.File]::WriteAllText($$lockerListPath, $$lockerText, (New-Object System.Text.UTF8Encoding $$false)); \
            $$selfLockers = @([pscustomobject]@{ name = '华青智能助手安装程序'; pid = [int]$$installerPid }); \
        ...
        if ($$lockers.Count -eq 0 -and $$installerSelfLock -and $$installerPid -gt 0) { $$lockers = @([pscustomobject]@{ name = '华青智能助手安装程序'; pid = [int]$$installerPid }) }; \
  ...
  ${If} $AionUiIsUpdated == "1"
    ${If} ${Silent}
      nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "& { \
        ...
        $$appDir = Join-Path $$env:APPDATA 'HQBuddy'; \
