...
function Read-AionUiAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'HQBuddy\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'HQBuddy\analytics.json')
  }
...
try {
  ...
  Show-ReportMessage ('华青智能助手安装报告已发送。' + [Environment]::NewLine + [Environment]::NewLine + '提示：按 [ Ctrl + C ] 可复制诊断详情，并将其发送给华青智能助手支持团队。' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('华青智能助手安装报告发送失败。' + [Environment]::NewLine + [Environment]::NewLine + '状态：' + $statusPath + [Environment]::NewLine + '日志：' + $log) 'Exclamation'
}
