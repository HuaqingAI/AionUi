...
async function backendSupportsAssistantDefinitions(): Promise<boolean> {
  try {
    ...
    console.warn('[HQBuddy] Failed to probe unified assistant detail support:', error);
    return false;
  }
}
...
async function markAssistantsMigrationDone(configFile: ConfigFile): Promise<void> {
  ...
  try {
    ...
    console.warn('[HQBuddy] failed to persist assistants migration flag', err);
  }
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[HQBuddy] Skipped override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[HQBuddy] Failed to apply builtin override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[HQBuddy] Applied ${applied} builtin disabled-state override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[HQBuddy] Builtin override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[HQBuddy] Skipped agent_id override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[HQBuddy] Failed to apply agent_id override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[HQBuddy] Applied ${applied} builtin agent_id override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[HQBuddy] Builtin agent_id override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
...
async function fetchCurrentBuiltinAgentIds(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[HQBuddy] Failed to fetch current builtin agent_id map:', error);
    return new Map();
  }
}
...
async function fetchAgentIdByRuntimeKey(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[HQBuddy] Failed to fetch agent runtime identity map:', error);
    return new Map();
  }
}
...
async function uploadLegacyAssistantRules(legacyAssistantIds: Set<string>): Promise<number> {
  ...
  try {
    ...
    console.error('[HQBuddy] Failed to read legacy assistant rules dir:', error);
    return 1;
  }
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      failed += 1;
      console.error(
        `[HQBuddy] Failed to upload legacy rule for '${ruleEntries[i].id}' (${ruleEntries[i].locale}):`,
        r.reason
      );
      return;
    }
  ...
  if (failed === 0) {
    if (uploaded > 0 || skipped > 0) {
      console.log(`[HQBuddy] Legacy rule upload: ${uploaded} uploaded, ${skipped} skipped`);
    }
  } else {
    console.error(`[HQBuddy] Legacy rule upload partial: ${failed}/${ruleEntries.length} failed`);
  }
  return failed;
}
...
export async function migrateAssistantsToBackend(configFile: ConfigFile): Promise<boolean> {
  if (process.env.AIONUI_SKIP_ELECTRON_MIGRATION === '1') {
    console.log('[HQBuddy] Assistant migration skipped (env flag set)');
    return false;
  }
  ...
  if (userAssistants.length > 0) {
    try {
      ...
      if (result.failed !== 0) {
        console.error(`[HQBuddy] Assistant migration partial: ${result.failed} failed`, result.errors);
        return false;
      }
      if (result.imported > 0 || result.skipped > 0) {
        console.log(`[HQBuddy] migrated ${result.imported} assistants (skipped ${result.skipped})`);
      }
    } catch (error) {
      console.error('[HQBuddy] Assistant migration failed:', error);
      return false;
    }
  }
