...
const INSTALLER_ERROR_SCENARIOS = [
  {
    ...
    code: 'E1001',
    message: 'HTHBuddy could not repair the installed uninstaller.',
    action: 'Close HTHBuddy, restart Windows if needed, then run this installer again.',
    diagnostics:
  ...
  },
  {
    ...
    code: 'E1002',
    message: 'The previous HTHBuddy uninstaller returned an error.',
    action:
  ...
  },
  {
    ...
    code: 'E1003',
    message: 'HTHBuddy could not remove or replace the previous installation directory.',
    action: 'Close HTHBuddy and any program using the install folder, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1010',
    message: 'HTHBuddy could not extract the application files correctly.',
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=extract-failed phase=extract method=zip missing=HTHBuddy.exe',
  },
  {
    id: 'disk-insufficient',
    defineName: 'AIONUI_E_DISK_INSUFFICIENT',
    code: 'E1020',
    message: 'HTHBuddy cannot continue because the target disk does not have enough free space.',
  ...
  },
  {
    ...
    code: 'E1030',
    message: 'HTHBuddy installed, but the bundled AionCore resources are incomplete.',
    action: 'Download a fresh installer and run it again.',
  ...
  },
  {
    ...
    code: 'E1031',
    message: 'HTHBuddy installation is incomplete because a required application file is missing.',
    action: 'Reinstall HTHBuddy or download a newer installer.',
  ...
  },
  {
    ...
    action: 'Download the HTHBuddy installer that matches this Windows architecture, then run it again.',
  ...
  },
  {
    ...
    code: 'E1050',
    message: 'Another HTHBuddy installer appears to still be active.',
  ...
  },
  {
    ...
    code: 'E1060',
    message: 'HTHBuddy found an invalid previous-install registry state.',
    action: 'Uninstall the old HTHBuddy from Windows Settings, then run this installer again.',
  ...
  },
  {
    ...
    code: 'E1070',
    message: 'HTHBuddy could not write the active-installer marker.',
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*HTHBuddy installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "HTHBuddy installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "HTHBuddy Failure MessageBox Smoke"
...
Section
  StrCpy $INSTDIR "$TEMP\\HTHBuddy-messagebox-smoke"
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`HTHBuddy installer failure ${code}`)) {
