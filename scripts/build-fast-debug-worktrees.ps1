...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\华青智能助手-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "华青智能助手-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "华青智能助手-$version"
